...
class mainLib
{
	...
	public function getLevelName($lid)
	{
		...
		$desc->execute([':id' => $lid]);
		$desc = $desc->fetch();
		if (!empty($desc["levelName"])) return $desc["levelName"];
		else return false;
	}
	...
	public function sendRateWebhook($modAccID, $levelID)
	{
		...
		if ($level['starStars'] == 1) $action = 0;
		elseif (($level['starStars'] < 5 and $level['starStars'] != 0) and !($level['starStars'] > 9 and $level['starStars'] < 20)) $action = 1;
		else $action = 2;
		...
		if ($level['requestedStars'] == 1) $action = 0;
		elseif (($level['requestedStars'] < 5 and $level['requestedStars'] != 0) and !($level['requestedStars'] > 9 and $level['requestedStars'] < 20)) $action = 1;
		else $action = 2;
	...
	public function sendSuggestWebhook($modAccID, $levelID, $difficulty, $stars, $featured, $auto, $demon)
	{
		...
		$IDField = [$this->webhookLanguage('levelIDTitle', $webhookLangArray), $level['levelID'], true];
		if ($stars == 1) $action = 0;
		elseif (($stars < 5 and $stars != 0) and !($stars > 9 and $stars < 20)) $action = 1;
		else $action = 2;
	...
	public function sendLogsListChangeWebhook($listID, $whoChangedID, $listData = [])
	{
		...
		if ($isDeleted || empty($listData)) {
			...
			elseif ($actions < 5 and $actions != 0 and !($newListData['starStars'] > 9 and $newListData['starStars'] < 20)) $action = 1;
			else $action = 2;
			...
			if ($listData['starStars'] != $newListData['starStars']) {
				...
				if ($oldReward == 1) $action = 0;
				elseif ($oldReward < 5 and $oldReward != 0 and !($listData['starStars'] > 9 and $listData['starStars'] < 20)) $action = 1;
				else $action = 2;
				...
				if ($newReward == 1) $action = 0;
				elseif ($newReward < 5 and $newReward != 0 and !($newListData['starStars'] > 9 and $newListData['starStars'] < 20)) $action = 1;
				else $action = 2;
			...
			if ($listData['countForReward'] != $newListData['countForReward']) {
				...
				if ($oldReward == 1) $action = 0;
				elseif ($oldReward < 5 and $oldReward != 0 and !($listData['countForReward'] > 9 and $listData['countForReward'] < 20)) $action = 1;
				else $action = 2;
				...
				if ($newReward == 1) $action = 0;
				elseif ($newReward < 5 and $newReward != 0 and !($newListData['countForReward'] > 9 and $newListData['countForReward'] < 20)) $action = 1;
				else $action = 2;
	...
	public function sendLogsMapPackChangeWebhook($packID, $whoChangedID, $packData = [])
	{
		...
		if ($isDeleted || empty($packData)) {
			...
			if ($newPackData['stars'] == 1) $action = 0;
			elseif (($newPackData['stars'] < 5 and $newPackData['stars'] != 0)) $action = 1;
			else $action = 2;
			$starsText = sprintf($this->webhookLanguage('requestedDesc' . $action, $webhookLangArray), $newPackData['stars']);
			if ($newPackData['coins'] == 1) $action = 0;
			elseif (($newPackData['coins'] < 5 and $newPackData['coins'] != 0)) $action = 1;
			else $action = 2;
			...
			foreach ($packLevels as &$packLevel) {
				...
				if ($getLevel['starStars'] == 1) $action = 0;
				elseif (($getLevel['starStars'] < 5 and $getLevel['starStars'] != 0) and !($getLevel['starStars'] > 9 and $getLevel['starStars'] < 20)) $action = 1;
				else $action = 2;
			...
			if ($packData['stars'] != $newPackData['stars']) {
				if ($packData['stars'] == 1) $action = 0;
				elseif (($packData['stars'] < 5 and $packData['stars'] != 0)) $action = 1;
				else $action = 2;
			...
			if ($packData['coins'] != $newPackData['coins']) {
				if ($packData['coins'] == 1) $action = 0;
				elseif (($packData['coins'] < 5 and $packData['coins'] != 0)) $action = 1;
				else $action = 2;
	...
	public function sendLevelsWarningWebhook($levelsYesterday, $levelsToday)
	{
		...
		if ($lYchar == 1) $action = 0;
		elseif ($lYchar < 5 and $lYchar != 0 and !($levelsYesterday > 9 and $levelsYesterday < 20)) $action = 1;
		else $action = 2;
		...
		if ($lTchar == 1) $action = 0;
		elseif ($lTchar < 5 and $lTchar != 0 and !($levelsToday > 9 and $levelsToday < 20)) $action = 1;
		else $action = 2;
	...
	public function sendAccountsWarningWebhook($accountsYesterday, $accountsToday)
	{
		...
		if ($lYchar == 1) $action = 0;
		elseif ($lYchar < 5 and $lYchar != 0 and !($accountsYesterday > 9 and $accountsYesterday < 20)) $action = 1;
		else $action = 2;
		...
		if ($lTchar == 1) $action = 0;
		elseif ($lTchar < 5 and $lTchar != 0 and !($accountsToday > 9 and $accountsToday < 20)) $action = 1;
		else $action = 2;
	...
	public function sendCommentsSpammingWarningWebhook($similarCommentsCount, $similarCommentsAuthors)
	{
		...
		if ($lYchar == 1) $action = 0;
		elseif ($lYchar < 5 and $lYchar != 0 and !($similarCommentsCount > 9 and $similarCommentsCount < 20)) $action = 1;
		else $action = 2;
